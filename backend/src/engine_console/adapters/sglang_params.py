...
def build_catalog() -> list[ParamSpec]:
    ...
    return [
        ...
        p("enable_hierarchical_cache", "bool", "memory", "Hierarchical KV cache (HiCache)",
          "Spill KV to host RAM/storage. Needs the radix cache enabled.", False, adv=True, mem=True),
        p("hicache_ratio", "float", "memory", "HiCache host/device ratio",
          "Host KV pool size relative to the device pool.", 2.0, lo=1.0, adv=True, mem=True),
        p("hicache_size", "int", "memory", "HiCache host size (GB)",
          "Host pool size in GB; overrides hicache-ratio.", None, lo=1, adv=True, mem=True),
