...
A single-workstation control plane and observability UI for **vLLM** and **SGLang** containers:
...
browser --loopback--> FastAPI (127.0.0.1:8791) --docker CLI (rootless)--> gateway (127.0.0.1:18000-18099) --> engine
   static UI (no build)     |  SQLite WAL, SSE, /metrics, OTLP                  (internal network, no ports, no egress)
                            `--HTTPS via host mitmproxy (opt-in)--> huggingface.co
```
...
Read next: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) (contract), [`docs/adr/`](docs/adr/) (12 decisions with
...
Prerequisites: rootless Docker with the `ai-lab` network, NVIDIA container toolkit, `uv`, Node 22 (tests only).
...
engines on an internal network with no ports or egress behind a per-instance gateway; console egress through the host
mitmproxy (opt-in, fail-closed); `--cap-drop ALL`. Known gaps: local processes are still admin unless
