...
The lab already runs engines as containers on the `ai-lab` network, pinned to a GPU by UUID. The console needs to create and destroy them without touching containers it did not create.

## Decision

`services/docker.py` builds `docker --context rootless run ...` as an argv list executed without a shell, with a timeout per call. Every container gets label `ai-lab.console=1`, `--security-opt no-new-privileges:true`, `--restart no`, GPUs by UUID, and a published port bound to `127.0.0.1` from range 18000-18099. `ps` and adoption filter on the label. Engines run with `HF_HUB_OFFLINE=1` by default.

## Consequences

- Matches the repo posture; the console cannot remove containers it did not label.
...
`ai-lab-engines` behind a per-instance gateway. Additional hardening on engine containers: `--cap-drop ALL`,
`--pids-limit 4096`, the HF cache mounted read-only (`HF_CACHE_READONLY` now defaults to true), image names
checked against `ENGINE_IMAGE_ALLOWLIST` (prefix match, so a pinned tag is not enforced, only the repo), and
`stop`/`rm`/`logs` verify the `ai-lab.console=1` label (plus role and owning-instance labels for gateways) before
